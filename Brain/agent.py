import numpy as np
from Brain.model import PolicyNetwork, QNetwork
import torch
...
class SAC:
    def __init__(self, **config):
        ...
        self.q_value_network1 = QNetwork(state_shape=self.state_shape, n_actions=self.n_actions).to(self.device)
        self.q_value_network2 = QNetwork(state_shape=self.state_shape, n_actions=self.n_actions).to(self.device)
        self.q_value_target_network1 = QNetwork(state_shape=self.state_shape,
                                                     n_actions=self.n_actions).to(self.device)
        self.q_value_target_network2 = QNetwork(state_shape=self.state_shape,
                                                     n_actions=self.n_actions).to(self.device)
    ...
    def store(self, state, action, reward, next_state, done):
        state = from_numpy(state).to("cuda")
        reward = torch.CharTensor([reward])
        action = torch.ByteTensor([action]).to('cuda')
        next_state = from_numpy(next_state).to('cuda')
